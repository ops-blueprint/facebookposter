...
Fully isolated from the Facebook Page automation on purpose:
  - separate dedupe log (content_pipeline/used_facts_instagram.json)
  - separate credentials (IG_USER_ID / IG_ACCESS_TOKEN, not FB_PAGE_*)
  - touches no file the Facebook workflows touch
so nothing here can break or race against the Facebook scheduling pipeline.
...
GitHub Actions cron (best-effort timing, same caveat we hit with Facebook's
old live-cron setup: can drift by hours, not exact).
...
REPO_SLUG = "ops-blueprint/facebookposter"  # for building the raw.githubusercontent.com URL
...
def main():
    ...
    if not args.dry_run and (not ig_user_id or not ig_token):
        print("Missing IG_USER_ID / IG_ACCESS_TOKEN in fb_auto_poster/.env", file=sys.stderr)
        sys.exit(1)
